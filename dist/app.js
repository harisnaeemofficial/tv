...
DuckieTV.controller('AboutCtrl', ["$scope", "$rootScope", "$q", "$http", "$filter", "$injector", "SettingsService", "StorageSyncService", "TorrentSearchEngines", "DuckieTorrent",
    function($scope, $rootScope, $q, $http, $filter, $injector, SettingsService, StorageSyncService, TorrentSearchEngines, DuckieTorrent) {
        ...
        var getStats = function() {
            ...
            ['userPreferences','torrenting.hashList','trakttv.token','trakttv.trending.cache','alarms','xem.mappings','snr.name-exceptions','snr.date-exceptions', 'fanart.cache'].map(function(key) { delete dumpLocalStorage[key]; });
            $scope.statistics.push({
...
.directive('clicktrap', ["SidePanelState", "$state",
    ...
    this.$get = ["$injector", "$rootScope", "$q",
        function($injector, $rootScope, $q) {
            ...
            return {
                search: function(str) {
                    return $injector.get('dialogs').create('templates/dialogs/subtitle.html', 'subtitleDialogCtrl', {
                        query: str
                    }, {
                        size: 'lg'
                    });
    ...
    function(dialogs, $http, $q) {
        ...
        var notifications = {
            '1.1.4': ["<li>Images: switched to Fanart.tv for all images after Trakt.tv ended support",
                ...
                "<li>standalone: (fix) linux minimize bug, (new) option to open add-new-torrent panel on torrentHost, (fix) linux and mac multy systray bug, (fix) defaults for first time users bug, (new) option to set the color of the system tray icon",
                ...
                "<li>searchEngines: (fix) remove Strike, KAT and torrentz as they are gone, (new) add 1337x, ExtraTorrent, Idope, IsoHunt, TorrentDownloads and Zooqle, (fix) bug in RarBG causing non-found results after 15 minutes, (fix) drop find random KAT mirror feature as rockaproxy is gone, (new) add 2160p search quality",
